...
Agora que nosso aplicativo é resistente a interrupções temporárias de rede, vamos ver como podemos escalá-lo horizontalmente para poder oferecer suporte a milhares de clientes simultâneos

- Escalonamento horizontal significa adicionar novos servidores à sua infraestrutura para lidar com novas demandas.
- Escalonamento vertical significa adicionar mais recursos (poder de processamento, memória, armazenamento) à sua infraestrutura existente.

Vamos usar todos os núcleos, disponíveis do host. Por padrão, o Node.JS executa seu código JavaScript em uma única thread, o que significa que, mesmo com uma CPU de 32 núcleos, apenas um núcleo será usado. Felizmente, o ***cluster*** ( um grupo de computadores ou servidores que trabalham juntos como um único sistema) ***module*** do Node.JS oferece uma maneira conveniente de criar um thread de trabalho por núcleo.
...
```jsx
// Vamos instalar o adaptador do cluster
npm install @socket.io/cluster-adapter
```

No main.js vamos adicionar as alterações:
...
Isso gerará um thread de trabalho por CPU disponível na sua máquina. Cada aba do navegador agora pode estar conectado a um servidor Socket.io diferente (3000, 3001, etc), e o adaptador está simplesmente encaminhando os chat message entre eles
